...
df.split<-split(df.raw, df.raw$Estacion)
...
#   newLL<-rep(NA,length(largeList))
#   for(i in 1:length(largeList)){
#     for(j in 1:length(names)){
#       newLL[i].mean.names[j] <- aggregate(largeList[i]$'names[j]', list(largeList[i]$Fecha), FUN=mean, na.rm=TRUE) 
#       newLL[i].max.names[j] <- aggregate(largeList[i]$'names[j]', list(largeList[i]$Fecha), FUN=max, na.rm=TRUE)
#     }
#   }
# }
