...
Aplicarem la funció _tTest_ creada anteriorment i realitzarem una comprovació amb la funció de R _t.test_.
...
En primer lloc, observem que en aquest cas d'anàlisi disposem de tres mostres en comptes de dues. La distribució de les tres mostres són molt similars amb una tendència simètrica en els tres casos. En el cas de les distribucions _'white'_ i _'black'_, aquestes són pràcticament calcades amb un punt central sobre el valor 2.75. Pel que fa al cas de la distribució que no correspon ni a gent blanca ni negra, aquest disposa d'un punt central amb un valor una mica major que estaria sobre el 2.8.
El que podem apreciar, és que aquesta tercera mostra, és en qüestions de volum és molt més reduïda (79 registres) respecte al volum de les altres dues que quasi estan formades per 4000 registres. En aquest sentit, respecte al teorema del límit central, totes tres serien considerades una mostra gran, i com a tal tindrien un comportament seguint una distribució de tipus normal. Les dues primeres, realitzant la comparativa real respecte la densitat teòrica, veiem que encaixen força bé, mentre que la tercera pot quedar una mica més allunyada. Un dels motius pel qual pot passar aquest fet, és degut a la petita mida d'aquesta mostra. És per aquest motiu, que decidirem obviar aquesta mostra a la següent hipòtesi a dur a terme.

## Funció

Es farà servir la funció realitzada a l’apartat 4.2. Justificació a l’apartat 6.5.

## Pregunta de recerca

Aquest pregunta es possible realitzar-la en els dos sentits, la nota mitjana poblacional _'colgpa'_ dels alumnes negres és superior a la nota mitjana poblacional _'colgpa'_ dels alumnes blancs, o a la inversa. Això és donat a que realment volem saber si té algun tipus relació la raça o no. Per tant, totes dues preguntes serien valides.
...
$$
$$
H_1:  \mu_{Black} > \mu_{White}
$$


## Justificació del test a aplicar

Com hem comentat a l'apartat 6.1, estem parlant de dues distribucions de tendència simètrica, d'una mida gran i que aplicant el teorema del límit central segueixen una distribució de tipus normal. A més a més, hem realitzat la comparativa real respecte la densitat teòrica i veiem que encaixen força bé en aquesta.
Per tant, tal com hem explicat anteriorment a l'apartat 5.5, podem aplicar la mateixa funció de test creada a l'apartat 4.2, donat que el resultat obtingut mitjançant _qnorm_ i _qt_ és molt similar.
...
Observant els resultats obtinguts en el càlcul del test a l'apartat anterior, podem veure que el valor p dins d'un rang de 0 a 1, és bastant gran i major que el nivell de significació. És per aquest motiu, que no podem rebutjar la hipòtesi nul·la. Així doncs, no podem afirmar que la nota mitjana poblacional _'colgpa'_ dels alumnes negres sigui superior a la nota mitjana poblacional _'colgpa'_ dels alumnes blancs. De fet, si retornem a la comparativa visual de l'apartat 6.1, podem veure que la distribució dels valors és gairebé igual en una mosta de 4000 registres en els dos casos.
...
La proporció d'atletes a la població és inferior al 5%?

## Hipòtesi nul·la i l'alternativa

$$
H_0: p_{Athletes} = 0.05
$$

$$
H_1:  p_{Athletes} < 0.05
$$

## Justificació del test a aplicar

Apliquem un test de la proporció sobre una mostra d'aquesta. Apliquem el contrast d'una mostra sobre la proporció, assumint l'aproximació de la distribució binomial a una distribució normal per a mostres de mida gran. 

## Càlcul

Aplicarem la funció _proportionTest_ creada anteriorment i realitzarem una comprovació amb la funció de R _prop.test_.
...
Observant els resultats obtinguts en el càlcul del test a l'apartat anterior, podem veure que el _p valor_ dins d'un rang de 0 a 1, és major que el nivell de significació. És per aquest motiu, que no podem rebutjar la hipòtesi nul·la. Així doncs, no podem afirmar que la proporció d'atletes a la població sigui inferior al 5%.
...
|-|---------------------------------------------------- | ----------------------------------------------------------------------------------------| -------------|
|P3.a| Iterval de confiança de la mitjana _'sat'_ al 95% i 90%   | (`r (ic95.sat$lim_inf)`, `r (ic95.sat$lim_sup)`, NC = `r (ic95.sat$NC)`) , (`r (ic90.sat$lim_inf)`, `r (ic90.sat$lim_sup)`, NC = `r (ic90.sat$NC)`) | L'interval de confiança de la mitjana al 95% és [`r (ic95.sat$lim_inf)`,`r (ic95.sat$lim_sup)`]. L'interval de confiança de la mitjana al 90% és [`r (ic90.sat$lim_inf)`,`r (ic90.sat$lim_sup)`]. |
|P3.b| Iterval de confiança de la mitjana _'colgpa'_ al 95% i 90%   | (`r (ic95.colgpa$lim_inf)`, `r (ic95.colgpa$lim_sup)`, NC = `r (ic95.colgpa$NC)`) , (`r (ic90.colgpa$lim_inf)`, `r (ic90.colgpa$lim_sup)`, NC = `r (ic90.colgpa$NC)`) | L'interval de confiança de la mitjana al 95% és [`r (ic95.colgpa$lim_inf)`, `r (ic95.colgpa$lim_sup)`]. L'interval de confiança de la mitjana al 90% és [`r (ic90.colgpa$lim_inf)`, `r (ic90.colgpa$lim_sup)`]. |
|P4| Hipòtesi: La nota mitjana poblacional _'colgpa'_ dels alumnes que són atletes és inferior a la nota mitjana poblacional _'colgpa'_ de la resta d'alumnes? | alfa : `r (ttest.athNoAth.colgpa.95[1])`, mean1 : `r (ttest.athNoAth.colgpa.95[2])`, mean2 : `r (ttest.athNoAth.colgpa.95[3])`, t : `r (ttest.athNoAth.colgpa.95[4])`, tcritical : `r (ttest.athNoAth.colgpa.95[5])`, pvalue : `r (ttest.athNoAth.colgpa.95[6])`, | Rebutgem la hipòtesi nul·la amb un pvalue molt petit. Afirmem que la nota mitjana poblacional _'colgpa'_ dels atletes és inferior a la de la resta dels alumnes.|
|P5| Hipòtesi: La nota mitjana poblacional _'colgpa'_ de les alumnes dones és superior a la nota mitjana poblacional _'colgpa'_ dels alumnes homes? | alfa : `r (ttest.femaleMale.colgpa.95[1])`, mean1 : `r (ttest.femaleMale.colgpa.95[2])`, mean2 : `r (ttest.femaleMale.colgpa.95[3])`, t : `r (ttest.femaleMale.colgpa.95[4])`, tcritical : `r (ttest.femaleMale.colgpa.95[5])`, pvalue : `r (ttest.femaleMale.colgpa.95[6])`, | Rebutgem la hipòtesi nul·la amb un pvalue molt petit i inferior al nivell de significació. Afirmem que la nota mitjana poblacional _'colgpa'_ de les alumnes dones és superior al dels alumnes homes.|
|P6| Hipòtesi: La nota mitjana poblacional _'colgpa'_ té relació segons la raça? | alfa : `r (ttest.blackWhite.colgpa.95[1])`, mean1 : `r (ttest.blackWhite.colgpa.95[2])`, mean2 : `r (ttest.blackWhite.colgpa.95[3])`, t : `r (ttest.blackWhite.colgpa.95[4])`, tcritical : `r (ttest.blackWhite.colgpa.95[5])`, pvalue : `r (ttest.blackWhite.colgpa.95[6])`, | Acceptem la hipòtesi nul·la amb un pvalue gran i superior al nivell de significació. Afirmem que la nota mitjana poblacional _'colgpa'_ no té cap mena de relació amb la raça dels alumnes.|
|P7| Hipòtesi: La proporció d'atletes a la població és inferior al 5%? | alfa : `r (proptest.athNoath.95[1])`, p : `r (proptest.athNoath.95[2])`, p0 : `r (proptest.athNoath.95[3])`, z : `r (proptest.athNoath.95[4])`, zcritical : `r (proptest.athNoath.95[5])`, pvalue : `r (proptest.athNoath.95[6])`, | No podem rebutjar la hipòtesi nul·la. Així doncs, no podem afirmar que la proporció d'atletes a la població sigui inferior al 5%.|
|P8|.    | .  | .  |
...
Després de realitzar un anàlisi complert del conjunt de dades, podem concloure diferents conceptes. En primer lloc podem confirmar que la nota mitjana poblacional _'colgpa'_ és inferior en aquells alumnes que són atletes de forma mitjana. A la vegada, aquesta nota també és superior de forma mitjana a les alumnes dones respecte els homes. Per altra banda, no hi ha cap mena de relació en que la nota mitjana poblacional _'colgpa'_ sigui superior/inferior respecte la raça dels alumnes. Finalment, podem contemplar que la proporció d'alumnes que també són atletes ronda el 5% de la població. 
